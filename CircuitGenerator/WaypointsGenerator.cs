...
namespace Racing.CircuitGenerator
{
    internal sealed class WaypointsGenerator
    {
        ...
        public IEnumerable<Point> GenerateSimpleCircuit(double width, double height)
        {
            yield return new Point(0.5 * width, minimumDistance);
            yield return new Point(width - minimumDistance, minimumDistance);
            yield return new Point(width - minimumDistance, 0.5 * height);
            yield return new Point(width - minimumDistance, height - minimumDistance);
            yield return new Point(0.5 * width, height - minimumDistance);
            yield return new Point(minimumDistance, height - minimumDistance);
            yield return new Point(minimumDistance, 0.5 * height);
            yield return new Point(minimumDistance, minimumDistance);
        }
        ...
        private List<Point> generateRandomPoints(int numberOfPoints, double width, double height)
        ...
        private void pushApart(IList<Point> points, int n)
        {
            for (var i = 0; i < n; i++)
            {
                pushApart(points);
        ...
        private void pushApart(IList<Point> points)
        {
        ...
        private List<Point> insertRandomMidpoints(IList<Point> dataSet)
        {
            ...
            for (int i = 0; i < dataSet.Count; ++i)
            {
                ...
                var displacement = new Point(0, 1).Rotate(random.NextDouble() * 2 * Math.PI);
        ...
        private static void trySelectPointOnTheLongestStraightAsStart(List<Point> dataSet)
        {
            var maxLength = -1.0;
            var maxIndex = -1;
